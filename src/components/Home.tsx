      ...
      <footer style={{ background: "#1A1A2E", padding: "64px 0 32px", borderTop: "1px solid rgba(255,255,255,.06)" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto", padding: "0 24px" }}>
          <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr 1fr 1fr", gap: 48, paddingBottom: 48, borderBottom: "1px solid rgba(255,255,255,.06)" }}>
            ...
            {[
              { title: "📚 Programs", links: [["Abacus Mastery", "/programs#prog-abacus-mastery"], ["Brain Gym", "/programs#prog-brain-gym"], ["Vedic Maths", "/programs#prog-vedic-maths"], ["Pre-Abacus", "/programs#prog-pre-abacus"], ["Tuitions", "/programs#prog-tuitions"]] },
              { title: "🏫 Company", links: [["About Us", "/"], ["Our Team", "/#team"], ["Gallery", "/#gallery"], ["Contact", "/contact"]] },
              { title: "🤝 Franchise", links: [["Why Partner?", "/franchise"], ["Investment Info", "/franchise"], ["Training & Support", "/franchise"], ["Apply Now", "/franchise#enquiry"]] },
            ].map(col => (
              <div key={col.title}>
                <div style={{ fontSize: 13, fontWeight: 800, letterSpacing: "0.1em", textTransform: "uppercase", color: "rgba(255,255,255,.35)", marginBottom: 20 }}>{col.title}</div>
                <ul style={{ listStyle: "none", display: "flex", flexDirection: "column", gap: 12 }}>
                  {col.links.map(([l, h]) => (
                    <li key={l}>
                      <Link href={h} style={{ fontSize: 14, color: "rgba(255,255,255,.55)", textDecoration: "none", fontWeight: 700, transition: "color .2s" }}
                        onMouseEnter={e => e.currentTarget.style.color = "#FFB347"}
                        onMouseLeave={e => e.currentTarget.style.color = "rgba(255,255,255,.55)"}>
                        {l}
                      </Link>
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", paddingTop: 28, flexWrap: "wrap", gap: 12 }}>
            <span style={{ fontSize: 13, color: "rgba(255,255,255,.25)", fontWeight: 700 }}>© 2024 Ascento Abacus. All rights reserved. Made with 💛 for young learners.</span>
            <div style={{ display: "flex", gap: 16 }}>
              {[["🌐 ascento.in", "#"], ["📘 Facebook", "#"], ["📸 Instagram", "#"]].map(([s, h]) => (
                <Link key={s} href={h} style={{ fontSize: 12, color: "rgba(255,255,255,.25)", textDecoration: "none", fontWeight: 800, transition: "color .2s" }}
                  onMouseEnter={e => e.currentTarget.style.color = "#FFB347"}
                  onMouseLeave={e => e.currentTarget.style.color = "rgba(255,255,255,.25)"}>
                  {s}
                </Link>
